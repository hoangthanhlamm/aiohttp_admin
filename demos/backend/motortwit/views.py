...
from aiohttp_admin.utils import json_response
...
class SiteHandler:
    ...
    async def timeline(self, request):
        ...
        messages = await self.mongo.message.find(query).sort('pub_date', -1).to_list(30)
    ...
    async def public_timeline(self, request):
        messages = await self.mongo.message.find().sort('pub_date', -1).to_list(30)
        return json_response({
            "messages": messages,
            "endpoint": request.match_info.route.name
        })
    ...
    async def login(self, request):
        ...
        if request.method == 'POST':
            ...
            if user is None:
                ...
                return redirect(request, 'timeline')
        return json_response({"error": error, "form": form})
    ...
    async def register(self, request):
        ...
        if request.method == 'POST':
            ...
            if not form['username']:
                ...
                profile_user = await self.mongo.user.find_one({'username': form['username']})
                print('User: ', profile_user)
                return json_response({"profile": profile_user})
        return json_response({"error": error, "form": form})
